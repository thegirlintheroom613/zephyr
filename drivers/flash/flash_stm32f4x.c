...
static
bool valid_range(off_t offset, uint32_t len)
{
	return offset >= 0 && (offset + len - 1 <= STM32F4X_FLASH_END);
}

static
int check_status(struct stm32f4x_flash *regs)
{
...
static
int wait_flash_idle(struct stm32f4x_flash *regs)
{
...
static
int write_byte(off_t offset, uint8_t val, struct stm32f4x_flash *regs)
{
	uint32_t tmp;
	int rc;
...
static
int erase_sector(uint16_t sector, struct stm32f4x_flash *regs)
{
	uint32_t tmp;
	int rc;
...
static
void flush_caches(struct stm32f4x_flash *regs)
{
...
static
int flash_stm32f4x_erase(struct device *dev, off_t offset, size_t len)
{
...
static
int flash_stm32f4x_read(struct device *dev, off_t offset, void *data,
	size_t len)
{
	if (!valid_range(offset, len)) {
		return -EINVAL;
...
static
int flash_stm32f4x_write(struct device *dev, off_t offset,
...
static
int flash_stm32f4x_write_protection(struct device *dev, bool enable)
{
	struct flash_priv *p = dev->driver_data;
...
static
struct flash_priv flash_data = {
	.regs = (struct stm32f4x_flash *) FLASH_R_BASE,
};

static const
struct flash_driver_api flash_stm32f4x_api = {
...
static
int stm32f4x_flash_init(struct device *dev)
{
	struct flash_priv *p = dev->driver_data;
